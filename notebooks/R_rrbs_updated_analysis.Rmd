...
```{r Targets and Bismark2DGE}
targets <- read.table('/gpfs/data/cbc/fedulov_alexey/porcine_rrbs/Fedulov_porcine_rrbs/dat/targets.txt', header = T, sep = '\t')
files <- targets$file             
yall <- readBismark2DGE(files, sample.names=targets$sample)

```
```{r Chromosomal factor levels}
...
#3890 4871 6043 3404 3144 7954 3614 2441 3255 2367 2856 4880 2389 3280 2201 1474 2562 2326  392  122 
```
```{r Getting gene IDs from Biomart}
...
```
```{r Organizing columns and getting M values}
...
# Research Question 6 - SMV normal vs HSMV normal (effect of diet and SMV is ref group) 
HSMVnormal_vs_SMVnormal$table$BH <- p.adjust(HSMVnormal_vs_SMVnormal$table$PValue, method = "BH")
HSMVnormal_vs_SMVnormal$table$bonferroni <- p.adjust(HSMVnormal_vs_SMVnormal$table$PValue, method = "bonferroni")


```
```{r More entrez gene IDs and results tables}
#get entrez IDs
...
counts_and_genes$hgnc_symbol <- NULL
```
```{r Encode not expressed scenarios}
...
# Question 6 
HSMVnormal_vs_SMVnormal_results <- data.frame(HSMVnormal_vs_SMVnormal$table) %>% tibble::rownames_to_column(var = 'ensembl_gene_id')
# add annotations
HSMVnormal_vs_SMVnormal_results <- left_join(HSMVnormal_vs_SMVnormal_results, entrez_ids, by = 'ensembl_gene_id')
# add the counts and M vals
HSMVnormal_vs_SMVnormal_results <- left_join(HSMVnormal_vs_SMVnormal_results, counts_and_genes, by = 'ensembl_gene_id')
# filter results based on on nominal p-value
HSMVnormal_vs_SMVnormal_results_filtered <- HSMVnormal_vs_SMVnormal_results  %>% dplyr::filter(PValue < 0.01) %>% arrange(logFC)
...
```
```{r Reshape data so we can inner-join the sample level information}
labels <- targets
...
#Question 6
#pivot longer and fix columns so that we can inner join the sample information:
HSMVnormal_vs_SMVnormal_results_filtered_long <- HSMVnormal_vs_SMVnormal_results_filtered %>% tidyr::pivot_longer(cols = starts_with('RH-'), names_to = "measurement", values_to = "value", values_transform = list(value = as.character))
HSMVnormal_vs_SMVnormal_results_filtered_long <- tidyr::separate(HSMVnormal_vs_SMVnormal_results_filtered_long, measurement, into = c('val1', 'sample', 'measurements'), sep = '-')
HSMVnormal_vs_SMVnormal_results_filtered_long$val1 <- NULL
HSMVnormal_vs_SMVnormal_results_filtered_long$sample <- paste0('RH-', HSMVnormal_vs_SMVnormal_results_filtered_long$sample)
HSMVnormal_vs_SMVnormal_results_filtered_long <- dplyr::inner_join(HSMVnormal_vs_SMVnormal_results_filtered_long, labels, by = 'sample')
#this bit sets the order of the ensembl_gene_id and hgnc_symbol to be sorted by logFC
HSMVnormal_vs_SMVnormal_results_filtered_long$ensembl_gene_id <- 
  factor(HSMVnormal_vs_SMVnormal_results_filtered_long$ensembl_gene_id, 
         levels = 
           (unique(HSMVnormal_vs_SMVnormal_results_filtered_long$ensembl_gene_id)[
             order(HSMVnormal_vs_SMVnormal_results_filtered_long$logFC, method = 'radix')
             ]))
HSMVnormal_vs_SMVnormal_results_filtered_long$hgnc_symbol <- 
  factor(HSMVnormal_vs_SMVnormal_results_filtered_long$hgnc_symbol, 
         levels = 
           (unique(HSMVnormal_vs_SMVnormal_results_filtered_long$hgnc_symbol)[
             order(HSMVnormal_vs_SMVnormal_results_filtered_long$logFC, method = 'radix')
             ]))
