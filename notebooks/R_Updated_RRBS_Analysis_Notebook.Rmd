...
save.image('/gpfs/data/cbc/fedulov_alexey/Fedulov_porcine_rrbs/metadata/jw/fedulov.RData')
```
```{r "what about minimal filtering where each sample has to have 1 methylated or unmethylated count? 68998 loci pass this"}
...
save.image('/gpfs/data/cbc/fedulov_alexey/Fedulov_porcine_rrbs/metadata/jw/fedulov.RData')
```
```{r "Calculate the M values and make an MDS plot"}
Me <- y$counts[, Methylation == "Me"]
Un <- y$counts[, Methylation == "Un"]

Coverage <- Me + Un
M <- log2(Me + .0001) - log2(Un + .0001)
...
#how many distinct ensembl gene IDs in genome? 31908
...
save.image('/gpfs/data/cbc/fedulov_alexey/Fedulov_porcine_rrbs/metadata/jw/fedulov.RData')
```
```{r "group loci based on which ensembl ID they are closest to and then sum counts -- gives us 7770 genes for our analysis"}
...
save.image('/gpfs/data/cbc/fedulov_alexey/Fedulov_porcine_rrbs/metadata/jw/fedulov.RData')
```
```{r "estimate dispersion and glmfit"}
...
M_pr <- as.data.frame(log2(Mepr + .0001) - log2(Unpr + .0001)) #same thing as log2((Mepr+.0001)/(Unpr+.0001))

Beta_pr <- as.data.frame(Mepr / (Unpr + Mepr+ .0001))
...
plotMDS((log(Mepr)), col=rep(1:6, each=3), main="log methylated counts", labels = c("RH-1_SMVnormal","RH-2_SMVnormal","RH-3_SMVnormal","RH-4_SMVischemic","RH-5_SMVischemic","RH-6_SMVischemic","RH-7_MVM","RH-8_MVM","RH-10_MVM","RH-11_HVM","RH-12_HVM","RH-14_HVM","RH-15_HSMVnormal","RH-16_HSMVnormal","RH-18_HSMVnormal","RH-19_HSMVischemic","RH-21_HSMVischemic","RH-22_HSMVischemic"))
...
ggplot(aes(logval), data=meth)+geom_density()+facet_wrap(~sample)+ggtitle('Methylated Counts')

unmeth <- counts_melt %>% dplyr::filter(measurement == 'Un')
unmeth$logval<- log(as.numeric(unmeth$value)+0.0001)
ggplot(aes(logval), data=unmeth)+geom_density()+facet_wrap(~sample)+ggtitle('Unmethylated Counts')

mval <- counts_melt %>% dplyr::filter(measurement == 'M_value')
#mval$logval<- log(as.numeric(mval$value)+0.0001)
ggplot(aes(as.numeric(value)), data=mval)+geom_density()+facet_wrap(~sample)+ggtitle('mvalues')

bval <- counts_melt %>% dplyr::filter(measurement == 'B_value')
#mval$logval<- log(as.numeric(mval$value)+0.0001)
ggplot(aes(as.numeric(value)), data=bval)+geom_density()+facet_wrap(~sample)+ggtitle('bvalues')


```
```{r}
#make 
#get log methylated counts (not bothering w offset since we dropped 0 methylated counts from data)
Mepr_log <- log(Mepr) 
#scale data
Mepr_log_scale <- scale(Mepr_log)
#calculate the distance
Mepr_pr_dist <- dist(Mepr_log_scale)

#hclust runs the hierarchical clustering
Mepr_pr_hclust <- hclust(Mepr_pr_dist)
...
k_clust_methyl <- find_k(Mepr_pr_hclust)

#look at the plot
plot(k_clust_methyl)
# 2 is the optimal number in this plot but 8 also looks ok

library(cluster)
sil_methyl <- silhouette(k_clust_methyl$pamobject)
plot(sil_methyl, border = NA)

```

```{r}
#try cutting the tree 
methyl_pr_cut <- cutree(Mepr_pr_hclust, k = 8) %>% tibble::enframe() %>% rename(ensembl_gene_id = name, cluster = value)
