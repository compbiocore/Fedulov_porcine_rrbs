...
```{r}
dplyr::filter(counts_and_genes, ensembl_gene_id == 'ENSSSCG00000017809')
```
...
```{r}
#fix columns:
SMVisch_vs_SMVnormal_results_filtered_long <- as.data.frame(SMVisch_vs_SMVnormal_results_filtered) %>% tidyr::pivot_longer(cols = starts_with('R'), names_to = "measurement", values_to = "value") 
SMVisch_vs_SMVnormal_results_filtered_long <- separate(SMVisch_vs_SMVnormal_results_filtered_long, measurement, into = c('val1', 'val2'), sep = 3)
SMVisch_vs_SMVnormal_results_filtered_long$val1 <- NULL
measurements <- gl(3,1,nrow(SMVisch_vs_SMVnormal_results_filtered_long), labels=c("Me_counts", "Un_counts", "M_value"))
SMVisch_vs_SMVnormal_results_filtered_long$measurement <- measurements
SMVisch_vs_SMVnormal_results_filtered_long <- separate(SMVisch_vs_SMVnormal_results_filtered_long, val2, into = c('sample', 'val3'), sep = '-')
SMVisch_vs_SMVnormal_results_filtered_long$sample <- str_replace(SMVisch_vs_SMVnormal_results_filtered_long$sample, '_M', '')
SMVisch_vs_SMVnormal_results_filtered_long$val3 <- str_replace(SMVisch_vs_SMVnormal_results_filtered_long$val3, 'value', 'M_value')
SMVisch_vs_SMVnormal_results_filtered_long$measurement <- SMVisch_vs_SMVnormal_results_filtered_long$val3
SMVisch_vs_SMVnormal_results_filtered_long$val3 <- NULL
SMVisch_vs_SMVnormal_results_filtered_long$sample <- paste0('RH-',SMVisch_vs_SMVnormal_results_filtered_long$sample)
SMVisch_vs_SMVnormal_results_filtered_long <- pivot_wider(SMVisch_vs_SMVnormal_results_filtered_long, names_from = 'measurement', values_from = 'value')
SMVisch_vs_SMVnormal_results_filtered_long <- inner_join(SMVisch_vs_SMVnormal_results_filtered_long, labels, by = 'sample')
SMVisch_vs_SMVnormal_results_filtered_long$M_value <- as.numeric(str_trim(SMVisch_vs_SMVnormal_results_filtered_long$M_value, side = c("both")))
SMVisch_vs_SMVnormal_results_filtered_long$Me <- as.numeric(str_trim(SMVisch_vs_SMVnormal_results_filtered_long$Me, side = c("both")))
SMVisch_vs_SMVnormal_results_filtered_long$Un <- as.numeric(str_trim(SMVisch_vs_SMVnormal_results_filtered_long$Un, side = c("both")))
...
```{r}
#then we plot
ggplot(data = (SMVisch_vs_SMVnormal_results_filtered_long %>% filter(group == "SMVischemic" | group == "SMVnormal")),
       mapping = aes(
         x = sample, 
         y = hgnc_symbol, 
         fill = M_value)) +
...
ggsave('/home/rstudio/dat/SMVisch_vs_SMVnormal_results_filtered_heatmap.png', device = 'png', height = 12)
