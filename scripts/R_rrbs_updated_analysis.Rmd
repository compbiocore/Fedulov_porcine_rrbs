...
SMVisch_vs_SMVnormal_results_filtered %>% dplyr::select(-matches('Me|Un'))

dplyr::select(SMVisch_vs_SMVnormal_results_filtered, contains("Me|Un"))

SMVisch_vs_SMVnormal_results_filtered_long <- SMVisch_vs_SMVnormal_results_filtered %>% dplyr::select(-matches('Me|Un')) %>% tidyr::pivot_longer(cols = starts_with('RH-'), names_to = "measurement", values_to = "value") 
SMVisch_vs_SMVnormal_results_filtered_long <- separate(SMVisch_vs_SMVnormal_results_filtered_long, measurement, into = c('val1', 'sample', 'measurements'), sep = '-')
...
```{r}
#then we plot
ggplot(data = (data.frame(SMVisch_vs_SMVnormal_results_filtered_long) %>% filter(group == "SMVischemic" | group == "SMVnormal")),
       mapping = aes(
         x = sample, 
         y = hgnc_symbol, 
