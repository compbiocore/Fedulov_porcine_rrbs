...
y_tss_slice <- y_tss_slice %>% unite("loci", seqnames:start, sep = '-')

```


```{r}
#biomart is missing a few ensembl to entrez gene ID conversions (for example, ENSSSCG00000037372)
#lets see if annotationhub can find them
#library(AnnotationHub)
#ah <- AnnotationHub()
#edb <- query(ah, c("EnsDb", "scrofa"))[["AH89550"]]
#gns <- genes(edb)
#head(gns)

#Nope, looks like annotationhub doesn't have it either
...
SMVisch_vs_SMVnormal_results <- left_join(SMVisch_vs_SMVnormal_results, y_tss_slice, by = c('loci' = 'loci'))
...
#add annotations from `y_gr_df`
HSMVisch_vs_HSMVnormal_results <- left_join(HSMVisch_vs_HSMVnormal_results, y_tss_slice, by = c('loci' = 'loci'))
# filter results based on on nominal p-value
...
#add annotations from `y_gr_df`
diffisch_vs_diffnormal_results <- left_join(diffisch_vs_diffnormal_results, y_tss_slice, by = c('loci' = 'loci'))
# filter results based on on nominal p-value
...
#add annotations from `y_gr_df`
MVM_vs_SMVischemic_results <- left_join(MVM_vs_SMVischemic_results, y_tss_slice, by = c('loci' = 'loci'))
# filter results based on on nominal p-value
...
#add annotations from `y_gr_df`
HVM_vs_HSMVischemic_results <- left_join(HVM_vs_HSMVischemic_results, y_tss_slice, by = c('loci' = 'loci'))
# filter results based on on nominal p-value
